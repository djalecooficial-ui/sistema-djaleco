...
import { Plus, Phone, Bot, Sparkles, Banknote } from "lucide-react";
...
export type Board = "whatsapp" | "site";
...
const COR_CLASSES: Record<string, { dot: string; ring: string }> = {
  blue: { dot: "bg-blue-500", ring: "border-blue-500/30" },
  yellow: { dot: "bg-yellow-500", ring: "border-yellow-500/30" },
  purple: { dot: "bg-purple-500", ring: "border-purple-500/30" },
  green: { dot: "bg-green-500", ring: "border-green-500/30" },
  red: { dot: "bg-red-500", ring: "border-red-500/30" },
  slate: { dot: "bg-slate-400", ring: "border-slate-400/30" },
  orange: { dot: "bg-orange-500", ring: "border-orange-500/30" },
  pink: { dot: "bg-pink-500", ring: "border-pink-500/30" },
};
    ...
    >
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          {unread && <span className="h-2 w-2 rounded-full bg-green-500 shrink-0 animate-pulse" />}
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          <Badge variant="outline" className={`text-[10px] ${ORIGEM_CLASS[c.origem ?? "outro"] ?? ORIGEM_CLASS.outro}`}>
      ...
      <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
        ...
        <div className="ml-auto flex items-center gap-1">
          ...
          {c.ai_enabled === false && (
            <Badge variant="outline" className="text-[10px] h-4 px-1 bg-amber-500/15 text-amber-700 dark:text-amber-400 border-amber-500/30">
      ...
      {c.last_message_preview && (
        <p className={`text-xs line-clamp-2 leading-snug ${unread ? "font-medium text-foreground" : "text-foreground/80"}`}>
          {truncate(c.last_message_preview, 60)}
        </p>
      )}
      <div className="text-[11px] text-muted-foreground">
        {lastDate ? formatDistanceToNow(new Date(lastDate), { addSuffix: true, locale: ptBR }) : "—"}
      </div>
    </Card>
  );
}

function DraggableCard({ c, onClick }: { c: Contato; onClick: () => void }) {
...
function DroppableColumn({
  ...
  return (
    <div ref={setNodeRef} className={`${className ?? ""} ${isOver ? "ring-2 ring-primary/50 bg-primary/5" : ""}`}>
      {children}
    </div>
  );
}
...
export function KanbanBoard({
  ...
  const [form, setForm] = useState({ nome: "", telefone: "", email: "", origem: "whatsapp", notas: "" });

  const contactsKey = ["crm_contacts_kanban", board];
  const columnsKey = ["crm_board_columns", board];
  ...
  const { data: contatos, isLoading } = useQuery<Contato[]>({
    ...
    queryFn: async () => {
      let query = supabase.from("crm_contacts").select("*").order("last_message_at", { ascending: false, nullsFirst: false });
      query = board === "site" ? query.eq("origem", "site") : query.neq("origem", "site");
      const [{ data, error }, { data: clientes }] = await Promise.all([query, supabase.from("clientes").select("telefone")]);
      if (error) throw error;
      const customerSuffixes = new Set(
        (clientes ?? []).map((c: any) => String(c.telefone ?? "").replace(/\D/g, "").slice(-8)).filter(Boolean),
      );
      return ((data ?? []) as Contato[]).map((c) => ({
        ...c,
        is_customer: customerSuffixes.has(String(c.telefone ?? "").replace(/\D/g, "").slice(-8)),
      }));
    },
  });
  ...
  useEffect(() => {
    const channel = supabase
      .channel(`crm_contacts_realtime_${board}`)
  ...
  function handleDragEnd(e: DragEndEvent) {
    ...
    if ((contact.status ?? columns?.[0]?.key) === overId) return;
    qc.setQueryData<Contato[]>(contactsKey, (old) => (old ?? []).map((c) => (c.id === contact.id ? { ...c, status: overId } : c)));
    moveStatus.mutate({ id: contact.id, status: overId });
  }
      ...
      <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd} onDragCancel={() => setActiveId(null)}>
        <div className="flex gap-4 overflow-x-auto pb-3 -mx-4 px-4 md:-mx-6 md:px-6">
          {(columns ?? []).map((col) => {
            ...
            const corClasses = COR_CLASSES[col.cor] ?? COR_CLASSES.slate;
            return (
              <DroppableColumn
                key={col.key}
                id={col.key}
                className={`shrink-0 w-[300px] md:w-[320px] rounded-lg border bg-muted/30 p-3 flex flex-col h-[calc(100vh-220px)] transition-colors ${corClasses.ring}`}
              >
                <div className="flex items-center justify-between mb-3 px-1">
                  <div className="flex items-center gap-2">
                    <span className={`h-2.5 w-2.5 rounded-full ${corClasses.dot}`} />
                ...
                <div className="space-y-2 flex-1 overflow-y-auto pr-1">
                  {isLoading ? (
                    ...
                    <p className="text-xs text-muted-foreground text-center py-6">Arraste aqui ou nenhum contato</p>
                  ) : (
                    items.map((c) => <DraggableCard key={c.id} c={c} onClick={() => navigate(`/crm/${c.id}`)} />)
                  )}
                </div>
              </DroppableColumn>
            );
